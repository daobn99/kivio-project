...
| 7 | [カテゴリー](#7-カテゴリー) | `categories` | catalog |
| 8 | [商品](#8-商品) | `products` | catalog |
| 9 | [商品画像](#9-商品画像) | `product_images` | catalog |
| 10 | [配送先住所](#10-配送先住所) | `addresses` | order |
| 11 | [カート](#11-カート) | `carts` | order |
| 12 | [カート明細](#12-カート明細) | `cart_items` | order |
| 13 | [注文](#13-注文) | `orders` | order |
| 14 | [注文明細](#14-注文明細) | `order_items` | order |
| 15 | [決済](#15-決済) | `payments` | order |
| 16 | [レビュー](#16-レビュー) | `reviews` | review |
| 17 | [チャットルーム](#17-チャットルーム) | `chat_rooms` | messaging |
| 18 | [チャットメッセージ](#18-チャットメッセージ) | `chat_messages` | messaging |
| 19 | [通知](#19-通知) | `notifications` | notification |
| 20 | [お気に入り](#20-お気に入り) | `wishlists` | review |
| 21 | [プラットフォーム設定](#21-プラットフォーム設定) | `platform_configs` | platform |
| 22 | [監査ログ](#22-監査ログ) | `audit_logs` | audit |
...
**概要：** バイヤーがセラーになるための申請。却下後の再申請は新規レコードを作成する。過去の申請履歴は保持。`reason`（および `review_comment`）は自由記述でPIIを含みうるため、退会後90日のユーザー匿名化バッチで当該フィールドを匿名化する。レコード自体は権限昇格の監査証跡として保持し、物理削除しない（RET-10）。

| 項目名（論理） | 項目名（物理） | Java型 | PK | FK | UQ | 必須 | 桁数 | デフォルト値 | 概要/備考 | 参照先 |
|---|---|:---:|:---:|:---:|:---:|:---:|:---:|---|---|---|
| セラー申請ID | id | `UUID` | ◯ | | | ◯ | - | auto | 主キー | - |
| 申請者ユーザーID | applicant_id | `UUID` | | ◯ | | ◯ | - | - | 申請したバイヤー | ユーザー.ユーザーID |
| 申請理由 | reason | `String` | | | | ◯ | - | - | セラー申請の動機・理由テキスト。1〜1000文字（アプリ層で制約）。退会後の匿名化で`(削除済み)`に置換 | - |
...
| 審査コメント | review_comment | `String` | | | | | - | null | 却下理由など。任意。退会後の匿名化で`(削除済み)`に置換 | - |
...
**テーブル名：** `addresses`  
**概要：** ユーザーが登録する配送先住所。複数登録可。注文確定時に`orders`テーブルへスナップショット保存するため、住所変更・削除の影響を受けない。ユーザー操作による削除は物理削除（`deleted_at`を持たない）。全項目がPIIのため、退会後90日のユーザー匿名化バッチで当該ユーザーの住所を物理削除する（RET-09）。
...
| デフォルトフラグ | is_default | `Boolean` | | | ◯ | ◯ | - | `false` | `true`はユーザーにつき1件のみ。アプリ側で付け替え＋部分UNIQUEインデックス`idx_addresses_user_default_unique`（`user_id` WHERE `is_default`）で担保 | - |
| 作成日時 | created_at | `Instant` | | | | ◯ | - | auto | レコード作成日時（UTC） | - |
| 更新日時 | updated_at | `Instant` | | | | ◯ | - | auto | レコード更新日時（UTC、自動更新） | - |
