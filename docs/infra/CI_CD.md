...
> 関連ファイル: `.github/workflows/ci.yml`、`.github/workflows/cd.yml`  
...
4. [CD ワークフロー（cd.yml）](#4-cd-ワークフローcdyml)
...
**本プロジェクトの CD の役割:**
- `main` ブランチへのマージ（＝リリース）が完了したら、デプロイ可能なイメージを自動生成する
...
└─────────────────────────┬───────────────────────────────┘
                          │ CI が main branch で成功
                          │ （PR・develop の CI 成功では CD は動かない）
                          ▼
┌─────────────────────────────────────────────────────────┐
...
| develop に push（PR マージ含む） | 動く | 動かない（結合ブランチのためイメージは作らない）|
| main に push（リリース） | 動く | CI 成功後に動く |

> **なぜ main だけで CD が動くのか**  
> デプロイ先を production のみとしているためです（`DEPLOYMENT.md §2`）。イメージは「デプロイされる可能性があるもの」だけを作れば十分で、develop でイメージを作っても使い道がありません。  
> リリースの流れ: `feature/*` → `develop`（結合・CI） → `main`（リリース・CD がイメージ生成） → 手動デプロイ
...
on:
  workflow_run:
    ...
    branches: [main]    # main ブランチで動いた場合のみ
```

`workflow_run` を使うことで「CI が成功したコミットのみ」をビルド対象にできます。PR や `develop` で CI が通っても CD は動きません。`main` への PR マージ（＝リリース）後に CI が通ったときだけ動きます。
...
| タグ | 例 | 目的 |
|---|---|---|
| `sha-xxxxxxx` | `sha-a1b2c3d` | **不変タグ**。どのコミットのビルドかを一意に識別する。デプロイ時は必ずこのタグを使う |
| `latest` | `latest` | **可変タグ**。main の最新ビルドを指す。中身が変わるためデプロイには使わない |

> **デプロイには必ず `sha-` タグを指定してください。** `latest` は次のリリースで中身が変わるため、「本番で何が動いているか」を追跡できなくなり、ロールバック先も特定できなくなります。
...
`cd.yml` の責務は **イメージの生成まで**です。Render / Vercel への反映は現時点では手動で行います。

手順は [DEPLOYMENT.md](./DEPLOYMENT.md) を参照してください。手動手順を確立してから自動化するという方針のため、`cd.yml` の末尾には自動化時の実装例（Render デプロイフック）をコメントで残しています。
