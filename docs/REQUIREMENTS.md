...
| # | 要件 |
|---|---|
| AUTH-01 | ユーザーはメールアドレス・パスワードで新規登録できる |
...
|---|---|---|---|
| ユーザー | `users` | メールアドレス、パスワード（ハッシュ）、Google ID、表示名、アバター画像URL、ロール、ステータス、メール確認済みフラグ | - |
...
| ショップ | `shops` | ショップ名、紹介文、ロゴ画像URL、公開ステータス | ユーザー・セラー（1:1） |
| ショップ配送ポリシー | `shop_shipping_policies` | 配送タイプ、固定送料金額、送料無料条件金額 | ショップ（1:1） |
| カテゴリー | `categories` | カテゴリー名、スラッグ、表示順序、親カテゴリー | カテゴリー（自己参照・親子） |
| 商品 | `products` | 商品名、説明文、価格（円）、在庫数、公開ステータス | ショップ（多:1）、カテゴリー（多:1） |
...
| **Phase 2** | 基盤構築・認証・ユーザー管理・セラー申請・ショップ・商品CRUD | AUTH, USER, SELLER, SHOP, PRD |
